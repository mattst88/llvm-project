...
#    elif SANITIZER_ALPHA
  // Linux/Alpha uses a 42-bit user VAS (TASK_SIZE = 0x40000000000).  With the
  // fixed shadow offset asan_mapping.h gives it, 0x30000000000, the layout is:
  //   LowMem:    [0x00000000000, 0x2ffffffffff]  (3 TiB)
  //   LowShadow: [0x30000000000, 0x35fffffffff]  (384 GiB)
  //   ShadowGap: [0x36000000000, 0x36bffffffff]
  //   HighShadow:[0x36c00000000, 0x37fffffffff]  (80 GiB)
  //   HighMem:   [0x38000000000, 0x3ffffffffff]  (512 GiB, stack near top)
...
#  elif defined(__alpha__)
  ...
  //
  // The pc is only probably readable: a jump through a bad pointer faults on
  // the fetch, and reading it here would fault a second time inside the signal
  // handler.  Check the page first.
  uptr pc = ucontext->uc_mcontext.sc_pc;
  if (!IsAccessibleMemoryRange(pc, sizeof(u32)))
    return SignalContext::Unknown;
